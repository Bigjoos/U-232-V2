...
HTML;
	$q1 = sql_query('SELECT id,name,image FROM categories order by id') or sqlerr();
	$i=0;
	while($a = mysql_fetch_assoc($q1)) {
		if($i%5 == 0 && $i>0)
			$HTMLOUT .="<br/>";
		$HTMLOUT .= "<label for=\"cat_{$a['id']}\"><img src=\"{$INSTALLER09['pic_base_url']}caticons/{$a['image']}\" alt=\"{$a['name']}\" title=\"{$a['name']}\" /><input type=\"checkbox\" name=\"cats[]\" id=\"cat_{$a['id']}\" value=\"{$a['id']}\" /></label>\n";
		$i++;
	}
$HTMLOUT .= <<<HTML
</td>
