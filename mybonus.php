...
$CURUSER[username] you have set the tracker <b>Free Leech !</b> <img src='{$INSTALLER09['pic_base_url']}smilies/w00t.gif' alt='w00t' title='W00t' /><br /><br />Remaining ".htmlspecialchars($_GET['norefund'])." points have been contributed towards the next freeleech period automatically!".
...
$HTMLOUT .="<table width='80%'><tr><td class='colhead' align='left' colspan='2'><h1>Success!</h1></td></tr><tr>".
"<td align='left' class='one'><img src='{$INSTALLER09['pic_base_url']}smilies/karma.gif' alt='good_karma' title='Good Karma' /></td><td align='left' class='one'><b>Congratulations! </b>
$CURUSER[username] you have set the tracker <b>Double Up !</b> <img src='{$INSTALLER09['pic_base_url']}smilies/w00t.gif' alt='w00t' title='W00t' /><br /><br />Remaining ".htmlspecialchars($_GET['norefund'])." points have been contributed towards the next Double upload period automatically!".
"<br /> click to go back to your <a class='altlink' href='{$INSTALLER09['baseurl']}/mybonus.php'>Karma Bonus Point</a> page.<br /><br />".
"</td></tr></table>";
echo stdhead($CURUSER['username'] . "'s Karma Bonus Points Page") . $HTMLOUT . stdfoot();
} else {
...
$HTMLOUT .="<table width='80%'><tr><td class='colhead' align='left' colspan='2'><h1>Success!</h1></td></tr><tr>".
"<td align='left' class='one'><img src='{$INSTALLER09['pic_base_url']}smilies/karma.gif' alt='good_karma' title='Good Karma' /></td><td align='left' class='one'><b>Congratulations! </b>
$CURUSER[username] you have set the tracker <b>Half Download !</b> <img src='{$INSTALLER09['pic_base_url']}smilies/w00t.gif' alt='w00t' title='W00t' /><br /><br />Remaining ".htmlspecialchars($_GET['norefund'])." points have been contributed towards the next Half download period automatically!".
"<br /> click to go back to your <a class='altlink' href='{$INSTALLER09['baseurl']}/mybonus.php'>Karma Bonus Point</a> page.<br /><br />".
"</td></tr></table>";
echo stdhead($CURUSER['username'] . "'s Karma Bonus Points Page") . $HTMLOUT . stdfoot();
} else {
...
$arr_free = mysql_fetch_assoc($res_free);
stderr('Success!', '<img src="pic/smilies/karma.gif" alt="good karma" /> <b>Congratulations '.$CURUSER['username'].'!!!</b> <img src="pic/smilies/karma.gif" alt="good karma" /><br />  you have ReAnimated the torrent <b><a class="altlink" href="details.php?id='.$arr_free['id'].'">'.htmlspecialchars($arr_free['name']).'</a></b>! Bringing it back to page one! <img src="pic/smilies/w00t.gif" alt="w00t" /><br /><br />
...
if ($CURUSER['class'] < UC_POWER_USER || $CURUSER['reputation'] >= 5000)
...
$reputation = $CURUSER['reputation'];
$rep = $reputation + $arr_points['menge'];
$bonuscomment = get_date( time(), 'DATE', 1 ) . " - " .$points. " Points for 100 rep points.\n " .$bonuscomment;
...
$bonuscomment = get_date( time(), 'DATE', 1 ) . " - " .$points. " Points for 1 years immunity status.\n " .$bonuscomment;
$immunity = (86400 * 365 + time());
...
$reputation = $CURUSER['reputation'];
if ($CURUSER['class'] < UC_POWER_USER || $CURUSER['reputation'] < 50)
...
$bonuscomment = get_date( time(), 'DATE', 1 ) . " - " .$points. " Points for user blocks access.\n " .$bonuscomment;
...
sql_query("UPDATE users SET invites = invites - 1, seedbonus = seedbonus + 200 WHERE id = ".sqlesc($userid)." AND invites = '$invites +1'") or sqlerr(__FILE__, __LINE__);
$mc1->delete_value('MyUser_'.$CURUSER['id']);
$mc1->delete_value('user'.$CURUSER['id']);
...
stderr("Error", "Time shall unfold what plighted cunning hides\n\nWho cover faults, at last shame them derides.");
$bonuscomment = get_date( time(), 'DATE', 1 ) . " - " .$points. " invites for bonus points.\n" .$bonuscomment;
sql_query("UPDATE users SET invites = invites - 1, freeslots = freeslots + 2 WHERE id = ".sqlesc($userid)." AND invites = '$invites +1'") or sqlerr(__FILE__, __LINE__);
$mc1->delete_value('MyUser_'.$CURUSER['id']);
$mc1->delete_value('user'.$CURUSER['id']);
...
$thief_rep = $CURUSER['reputation'];
$thief_bonus = $CURUSER['seedbonus'];
...
while($ar = mysql_fetch_assoc($qr)){
  ...
  $mc1->update_row(false, array('reputation' => $new_rep, 'seedbonus' => $new_bonus));   
  $mc1->commit_transaction(900); // 15 mins
  $mc1->begin_transaction('user'.$ar['id']);
  $mc1->update_row(false, array('reputation' => $new_rep, 'seedbonus' => $new_bonus));   
  $mc1->commit_transaction(300); // 5 mins
  // end
}
...
$mc1->delete_value('user'.$CURUSER['id']);
write_bonus_log($CURUSER["id"], $donation, $type = "freeleech");
$Remaining = $arr_points['points'] - $arr_points['pointspool'];
$msg = $CURUSER['username']. " Donated ".$donation." karma point".($donation > 1?'s':'')." into the freeleech contribution pot ! * Only [b]".$Remaining."[/b] more karma point".($Remaining > 1?'s':'')." to go! * [color=green][b]Freeleech contribution:[/b][/color] [url={$INSTALLER09['baseurl']}/mybonus.php]". $donation ."/".$points.'[/url]';
autoshout($msg);
...
$msg = $CURUSER['username']. " Donated ".$donation." karma point".($donation > 1?'s':'')." into the double upload contribution pot ! * Only [b]".$Remaining."[/b] more karma point".($Remaining > 1?'s':'')." to go! * [color=green][b]Double upload contribution:[/b][/color] [url={$INSTALLER09['baseurl']}/mybonus.php]". $donation ."/".$points.'[/url]';
autoshout($msg);
...
$free_time = (7 * 86400 + time());
$bonuscomment = get_date( time(), 'DATE', 1 ) . " - " .$points. " Points to Reanimate torrent: ".$arr_free['name'].".\n " .$bonuscomment;
sql_query('UPDATE users SET bonuscomment = '.sqlesc($bonuscomment).', seedbonus = '.sqlesc($seedbonus).' WHERE id = '.$CURUSER['id']) or sqlerr(__FILE__, __LINE__);
sql_query('UPDATE torrents SET bump = \'yes\', free='.sqlesc($free_time).', added = '.sqlesc(time()).' WHERE id = '.sqlesc($torrent_number)) or sqlerr(__FILE__, __LINE__);
$mc1->delete_value('MyUser_'.$CURUSER['id']);
$mc1->delete_value('user'.$CURUSER['id']);
...
sql_query("UPDATE users SET class = ".UC_VIP.", vip_added = 'yes', vip_until = '$vip_until', seedbonus = ".sqlesc($seedbonus).", bonuscomment = ".sqlesc($bonuscomment)." WHERE id = ".sqlesc($userid)."") or sqlerr(__FILE__, __LINE__);
$mc1->delete_value('MyUser_'.$CURUSER['id']);
$mc1->delete_value('user'.$CURUSER['id']);
...
$usernamegift = strip_tags(trim($_POST['username']));
...
function write_bonus_log($userid, $amount, $type){
  ...
  break;
  case ($gets['id'] == 9):
  $HTMLOUT .="<tr><td align='left' class='".$class."'><form action='{$INSTALLER09['baseurl']}/mybonus.php?exchange=1' method='post'><input type='hidden' name='option' value='".$gets['id']."' /> <input type='hidden' name='art' value='".$gets['art']."' /><h1><font color='#CECFF3'>".$gets['bonusname']."</font></h1>".$gets['description']."</td><td align='center' class='".$class."'>min.<br />".$gets['points']."</td>";
  break;
  case ($gets['id'] == 10):
  ...
  break;
  case ($gets['id'] == 34):
  $HTMLOUT .="<tr><td align='left' class='".$class."'><form action='{$INSTALLER09['baseurl']}/mybonus.php?exchange=1' method='post'><input type='hidden' name='option' value='".$gets['id']."' /> <input type='hidden' name='art' value='".$gets['art']."' /><h1><font color='#CECFF3'>".$gets['bonusname']."</font></h1>".$gets['description']."<br /><br />Enter the <b>ID number of the Torrent:</b> <input type='text' name='torrent_id' size='4' maxlength='8' /> you would like to bump.</td><td align='center' class='".$class."'>min.<br />".$gets['points']."</td>";
  break;
  default:
  $HTMLOUT .="<tr><td align='left' class='".$class."'><form action='{$INSTALLER09['baseurl']}/mybonus.php?exchange=1' method='post'><input type='hidden' name='option' value='".$gets['id']."' /> <input type='hidden' name='art' value='".$gets['art']."' /><h1><font color='#CECFF3'>".$gets['bonusname']."</font></h1>".$gets['description']."</td><td align='center' class='".$class."'>".$gets['points']."</td>";
  }
