...
loggedinorreturn();
    ...
    header( "Location: {$INSTALLER09['baseurl']}/multiupload.php" );
    ...
    stderr("Oops", "No NFO! for #1 torrent OR NFO #1 is too big! Max 65,535 bytes.");

    $f2 = $_FILES["file2"];
    $nfofile2 = $_FILES['nfo2'];
    $fname[] = unesc($f2["name"]);
    if ($nfofile2['size'] > 65535)
    stderr("Error", "No NFO! for #2 torrent OR NFO #2 is too big! Max 65,535 bytes.");

    $f3 = $_FILES["file3"];
    $nfofile3 = $_FILES['nfo3'];
    $fname[] = unesc($f3["name"]);
    if ($nfofile3['size'] > 65535)
    stderr("Oops", "No NFO! for #3 torrent OR NFO #3 is too big! Max 65,535 bytes.");

    $f4 = $_FILES["file4"];
    $nfofile4 = $_FILES['nfo4'];
    $fname[] = unesc($f4["name"]);
    if ($nfofile4['size'] > 65535)
    stderr("Oops", "No NFO! for #4 torrent OR NFO #4 is too big! Max 65,535 bytes.");

    $f5 = $_FILES["file5"];
    $nfofile5 = $_FILES['nfo5'];
    $fname[] = unesc($f5["name"]);
    if ($nfofile5['size'] > 65535)
    stderr("Oops", "No NFO! #5 torrent OR NFO #5 is too big! Max 65,535 bytes.");
    ...
    foreach ($a as $k) {
        ...
        if (!isset($dd[$k]))
            stderr("Oops", "dictionary is missing key(s)");
        if (isset($t)) {
            if ($dd[$k]["type"] != $t)
                stderr("Oops", "invalid entry in dictionary");
            $ret[] = $dd[$k]["value"];
        } else
            $ret[] = $dd[$k];
    }
...
function dict_get($d, $k, $t)
{
    ...
    stderr("Oops", "You must select a category to put ALL the torrent in!");
   ...
   $totallen = dict_get($info[$i], "length", "integer");
    if (isset($totallen)) {
        ...
        foreach ($flist as $fn) {
            ...
            foreach ($ff as $ffe) {
                if ($ffe["type"] != "string")
                    stderr("Oops", "filename error on torrent #$i");
                $ffa[] = $ffe["value"];
            }
            if (!count($ffa))
                stderr("Oops", "filename error");
            $ffe = implode("/", $ffa);
    ...
    $ret = mysql_query("INSERT INTO torrents (search_text, filename, owner, visible, anonymous, vip, info_hash, name, size, numfiles, type, descr, ori_descr, category, save_as, added, last_action, nfo) VALUES (" . implode(",", array_map("sqlesc", array(searchfield("$first $second $third"), $fname[$i], $CURUSER["id"], "no", $anonymous, $vip, $infohash[$i], $torrent[$i][1], $totallen, count($filelist[$i]), $type, $descr, $descr, $cat[$i], $dname[$i]))) . ", '" . time() . "', '" . time() . "', $nfo[$i])");
    if (!$ret) {
        if (mysql_errno() == 1062)
            stderr("Oops", "#$i torrent was already uploaded!");
        stderr("Oops", "mysql puked: " . mysql_error());
    }
    ...
    $message = "[url={$INSTALLER09['baseurl']}/multidetails.php?id1=$ids[0]&id2=$ids[1]&id3=$ids[2]&id4=$ids[3]&id5=$ids[4]]Multiple Torrents were just uploaded! Click here to see them[/url] - Anonymous User";
    else
    $message = "[url={$INSTALLER09['baseurl']}/multidetails.php?id1=$ids[0]&id2=$ids[1]&id3=$ids[2]&id4=$ids[3]&id5=$ids[4]]Multiple Torrents were just uploaded! Click here to see them[/url]  Uploaded by ".htmlspecialchars($CURUSER["username"])."";
    // ///////////////////////////END///////////////////////////////////
    // //////new torrent upload detail sent to shoutbox//////////
    autoshout($message);
    // ///////////////////////////end///////////////////////////////////
