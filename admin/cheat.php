...
if (isset($_POST["nowarned"]) && $_POST["nowarned"] == "nowarned") {
    ...
    if (!empty($_POST["remove"])) {
        sql_query("DELETE FROM cheatdetect WHERE id IN (" . implode(", ", $_POST["remove"]) . ")") or sqlerr(__FILE__, __LINE__);
    }
...
$res = sql_query("SELECT COUNT(*) FROM cheatdetect") or sqlerr();
$row = mysql_fetch_array($res);
$count = $row[0];
$perpage = 15;
$pager = pager($perpage, $count, "staffpanel.php?tool=cheat&amp;action=cheat&amp;");


$HTMLOUT .="<form action='staffpanel.php?tool=cheat&amp;action=cheat' method='post'>
...
$res = sql_query("SELECT * FROM cheatdetect ORDER BY time DESC ".$pager['limit']."") or sqlerr(__FILE__, __LINE__);
while ($arr = mysql_fetch_assoc($res)) {
    ...
    $cheater = "<b><a href='{$INSTALLER09['baseurl']}/userdetails.php?id=$aaa[id]'>$aaa[username]</a></b> Cheater detected<br /><br />Torrent ID <b>".htmlspecialchars($arr['torrentid'])."</b><br />Suspicion <b>".htmlspecialchars($arr['suspicion'])."</b><br />Detected client <b>".htmlspecialchars($arr['detectedclient'])."</b><br />Data sent <b>".htmlspecialchars($arr['data'])."</b>";

    $HTMLOUT .="<tr><td class=\"table\" width=\"10\" align=\"center\">$arr[id]</td>
    <td class=\"table\" align=\"left\"><a href=\"javascript:klappe('a1$arr[id]')\">$aaa[username]</a> - Added: ".get_date($arr['time'], 'DATE')."
...
echo stdhead('Cheat detection') . $HTMLOUT . stdfoot();
die;
?>