...
if ((isset($_GET["deal_with_report"])) || (isset($_POST["deal_with_report"]))) {
   ...
   " . ($CURUSER["class"] == UC_MAX ? "<td class='colhead' align='center' valign='top'>{$lang['reports_delete']}</td>" : "") . "</tr>";
    ...
    while ($arr_info = mysql_fetch_assoc($res_info)) {
        ...
        if ($arr_info["reporting_type"] != "") {
            switch ($arr_info["reporting_type"]) {
                ...
                case "Offer":
                    ...
                    $arr_who2 = mysql_fetch_assoc($res_who2);
                    $link_to_thing = "<a class='altlink' href='viewoffers.php?id={$arr_info['reporting_what']}&amp;off_details=1'><b>" . htmlspecialchars($arr_who2['username']) . "</b></a>";
                    break;
                case "Torrent":
                ...
                case "Hit_And_Run":
                    ...
                    $arr_who2 = mysql_fetch_assoc($res_who2);
                    $link_to_thing = "<b>{$lang['reports_user']}</b> <a class='altlink' href='userdetails.php?id={$arr_info['reporting_what']}&amp;completed=1'><b>{$arr_who2['username']}</b></a><br />{$lang['reports_hit']}<br /> <a class='altlink' href='details.php?id={$arr_info['2nd_value']}&amp;page=0#snatched'><b>" . htmlspecialchars($arr_who2['name']) . "</b></a>";
                    break;
                case "Post":
        ...
        <td align='center' valign='middle' class='$class'>{$checkbox}</td>" . ($CURUSER["class"] == UC_MAX ? "<td align='center' valign='middle' class='$class'><a class='altlink' href='staffpanel.php?tool=reports&amp;action=reports&amp;id={$arr_info['id']}&amp;delete=1'><font color='red'>{$lang['reports_delete']}</font></a></td>" : "") . "</tr>\n";
        // ===how was it delt with?
        if ($arr_info['how_delt_with'])
        $HTMLOUT .="<tr>
        <td colspan='" . ($CURUSER["class"] == UC_MAX ? "6" : "5") . "' class='$class' align='left'><b>{$lang['reports_with']} " . $arr_who['username'] . ":</b> " .get_date($arr_info['when_delt_with'], 'LONG',0,1) . "</td></tr>
        <tr><td colspan='" . ($CURUSER["class"] == UC_MAX ? "6" : "5") . "' class='$class' align='left'>" .htmlspecialchars($arr_info['how_delt_with']) . "<br /><br /></td></tr>";
    }
}
$HTMLOUT .="</table>";
if ($count > '0') {
