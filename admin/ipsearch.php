 ...
 elseif ($order == "last_access")
  ...
  $HTMLOUT .= begin_frame("".htmlspecialchars($count)." users have used the IP: ".htmlspecialchars($ip)." (".htmlspecialchars($addr).")", True);
   ...
   $HTMLOUT .= $pager['pagertop'];
	 ...
	 $HTMLOUT .= "<tr>
	  <td class='colhead'><a href='{$INSTALLER09['baseurl']}/staffpanel.php?tool=ipsearch&amp;action=ipsearch&amp;ip=$ip&amp;mask=$mask&amp;order=username'>Username</a></td>".
		"<td class='colhead'>Ratio</td>".
		"<td class='colhead'><a href='{$INSTALLER09['baseurl']}/staffpanel.php?tool=ipsearch&amp;action=ipsearch&amp;ip=$ip&amp;mask=$mask&amp;order=email'>Email</a></td>".
	...
	while ($user = mysql_fetch_assoc($res))
	{
	  ...
	  if ($user['last_ip'])
	  {
		...
		if ($user["invitedby"] > 0)
		{
	   	...
	   	$HTMLOUT .= "<tr>
	   	<td><b><a href='{$INSTALLER09['baseurl']}/userdetails.php?id=" . $user['id'] . "'></a></b>" . format_username($user) . "</td>".
