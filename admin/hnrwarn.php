...
$stdfoot = array(/** include js **/'js' => array('wz_tooltip'));
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if($_uids == 0 || count($_uids) == 0)
          ...
          $mc1->delete_value('MyUser_'.$_uids);
			...
			elseif($act == "unwarn")
			{
				$sub = "Hit and Run Warn removed";
				$body = "Hey, your Hit and Run warning was removed by ".$CURUSER["username"]."\nPlease keep in your best behaviour from now on.";
				$mc1->delete_value('user'.$_uids);
        $mc1->delete_value('MyUser_'.$_uids);
				$pms = array();
				foreach ($_uids as $id)
				...
				if(count($pms))
				{
					...
					$q1 = sql_query("UPDATE users set hnrwarn='no', modcomment=CONCAT(".sqlesc(get_date( time(), 'DATE', 1 ) . " - Hit and Run Warning removed by " . $CURUSER['username']."\n").",modcomment) WHERE id IN (".join(",",$_uids).")") or ($q2_err = mysql_error());
					if($g && $q1)
					{
						header("Refresh: 2; url=".$r);
    ...
    $title = "Disabled users";
	  $link = "<a href=\"staffpanel.php?tool=hnrwarn&amp;action=hnrwarn&amp;?do=warned\">Hit and Run warned users</a>";
		break;
		case "hnrwarn" : $query = "SELECT id, username, class, downloaded, uploaded, IF(downloaded>0, round((uploaded/downloaded),2), '---') as ratio, warn_reason, hnrwarn, added, last_access FROM users WHERE hnrwarn='yes' ORDER BY last_access DESC, hnrwarn DESC ";
	...
	$HTMLOUT .= begin_frame($title."&nbsp;[<font class=\"small\">total - ".$count." user".($count>1 ? "s" : "")."</font>] - ".$link);
		...
		<tr>    	
			...
			{
			$tip = ($do == "hnrwarn" ? "Hit and run Warned for : ".$a["warn_reason"]."<br />" : "Disabled for ".$a["disable_reason"]);
				$HTMLOUT .="<tr>
