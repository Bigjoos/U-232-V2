 ...
 **/
	...
	$valid_do = array("stats","torrents","fls","irc","top_idle","top_uploaders","top_posters","top_torrents");
	...
	require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'include'.DIRECTORY_SEPARATOR.'bittorrent.php');
		...
		if($_do == "stats")
		{
			$q = sql_query("SELECT id, username, last_access, downloaded, uploaded, added, status, warned, disable_reason, warn_reason FROM users WHERE username = ".sqlesc($_user)) or exit(mysql_error());
			if(mysql_num_rows($q) == 1)
			{
				$a = mysql_fetch_assoc($q);
				$txt = $a["username"] ." is ".((time() - $a["last_access"]) < 300 ? "online" : "offline")."\nJoined - ".get_date($a["added"],'LONG',0,1)."\nLast seen - ".get_date($a["last_access"],'DATE',0,1)."\nDownloaded - ".mksize($a["downloaded"])."\nUploaded - ".mksize($a["uploaded"])."\n";
			...
			unset($txt); unset($a); unset($q);
		}
		elseif($_do == "torrents")
		{
			$q = sql_query("SELECT count(p.id) as count, p.seeder,p.agent,p.port,p.connectable, u.username FROM peers as p LEFT JOIN users as u ON u.id = p.userid WHERE u.username=".sqlesc($_user)." GROUP BY p.seeder") or exit(mysql_error());
		 ...
		 $act['seed'] = $act['leech'] = 0;
			while ($a = mysql_fetch_assoc($q))
			{
				...
				$act[$key] = $a["count"];
				$agent = $a["agent"]; $port = $a["port"]; $con = $a["connectable"]; $user = $a["username"];
			}
		...
		elseif($_do == "fls")
		{
			...
			$q = sql_query("SELECT id,username,last_access ,supportfor FROM users WHERE support = 'yes' ORDER BY added desc") or exit(mysql_error());
			$txt = "";
			while($a = mysql_fetch_assoc($q))
			{
				$txt .= $a["username"]." - status ".((time() - $a["last_access"]) < 300 ? "online" : "offline"). " | Support for ".$a["supportfor"]."\n";
				unset($support);
			}
			echo($txt);
				unset($_fls);unset($a);unset($q);unset($txt);
		}
		elseif($_do == "irc")
		{
			$q = sql_query("SELECT onirc, irctotal,username FROM users WHERE username = ".sqlesc($_user)) or exit(mysql_error());
			if(mysql_num_rows($q) == 0)
				exit("User \"".$_user."\" not found!");
			$a = mysql_fetch_assoc($q);
			$txt = $a["username"]." ".($a["irctotal"] == 0 ? "never been on irc" : "has idled on irc ".calctime($a["irctotal"]))."\nAnd now he ".($a["onirc"] == "yes" ? "is" : "isn't")." on irc";
			echo($txt);
			unset($a);unset($q);unset($txt);
		}
		elseif($_do == "top")
		{
			switch($_type)
			...
			$i= 1;
			$q = sql_query($_q) or exit(mysql_error());
			while($a = mysql_fetch_assoc($q)) {
