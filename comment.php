...
if (isset($_GET['type'])) {
    ...
    switch ($_GET['type']) { 
          ...
          case 'offer':
          $sql_1 = 'offer FROM offers';
          $name = 'offer';
          $extra_link = '&offer_details';
          $table_type = $locale.'s';
        	break;
...
if ($action == 'add') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        sql_query("INSERT INTO comments (user, $locale, added, text, ori_text, anonymous) VALUES (".$CURUSER["id"].",$id, ".time().", " . sqlesc($text) .
        "," . sqlesc($text) . ", $anon)");
        ...
        $cpm = sql_query("SELECT commentpm FROM users WHERE id = {$arr['owner']}") or sqlerr(__FILE__, __LINE__);
        $cpm_r = mysql_fetch_assoc($cpm);
