...
{
$langs = isset($_POST["language"]) ? safe($_POST["language"]) : "";
if (empty($langs))
...
$date = time();
$owner = $CURUSER["id"];
sql_query("INSERT INTO subtitles (name , filename,imdb,comment, lang, fps, poster, cds, added, owner ) VALUES (" .implode(",", array_map("sqlesc", array($releasename, $filename,$imdb,$comment, $langs, $fps,$poster, $cd , $date, $owner ))).")") or sqlerr(__FILE__, __LINE__);
...
if ($CURUSER["id"] != $arr["owner"] && $CURUSER['class'] < UC_MODERATOR)
	...
	sql_query("UPDATE subtitles SET " . join(",", $updateset) . " WHERE id ={$id} ") or sqlerr(__FILE__, __LINE__);
	header("Refresh: 0; url=subtitles.php?mode=details&id=$id");
...
print stdhead("".($mode == "upload" ? "Upload new Subtitle" : "Edit subtitle ".$arr["name"]."")."") . $HTMLOUT . stdfoot();
}
//==Delete subtitle
elseif ($mode == "delete")
{
...
if ($arr["lang"] == "eng")
$langs = "<img src=\"pic/flag/england.gif\" border=\"0\" alt=\"English\" title=\"English\" />";
elseif ($arr["lang"] == "swe")
$langs = "<img src=\"pic/flag/sweden.gif\" border=\"0\" alt=\"Swedish\" title=\"Swedish\" />";
elseif ($arr["lang"] == "dan")
$langs = "<img src=\"pic/flag/denmark.gif\" border=\"0\" alt=\"Danish\" title=\"Danish\" />";
elseif ($arr["lang"] == "nor")
$langs = "<img src=\"pic/flag/norway.gih\" border=\"0\" alt=\"Norwegian\" title=\"Norwegian\" />";
elseif ($arr["lang"] == "fin")
$langs = "<img src=\"pic/flag/finland.gif\" border=\"0\" alt=\"Finnish\" title=\"Finnish\" />";
elseif ($arr["lang"] == "spa")
$langs = "<img src=\"pic/flag/spain.gif\" border=\"0\" alt=\"Spanish\" title=\"Spanish\" />";
elseif ($arr["lang"] == "fre")
$langs = "<img src=\"pic/flag/france.gif\" border=\"0\" alt=\"French\" title=\"French\" />";
else
$langs = "<b>Unknown</b>";
...
<img src='".$arr["poster"]."' width='150' height='195' alt='".$arr["name"]."' />
<br /><br />
...
$HTMLOUT .="<tr><td align='left'><fieldset><legend><b>Comment</b></legend>&nbsp;".safe($arr["comment"])."</fieldset></td></tr>";
}
$HTMLOUT .="<tr><td align='left'>FPS :&nbsp;<b>".($arr["fps"] == 0 ? "Unknown" : safe($arr["fps"]))."</b></td></tr>
<tr><td align='left'>Cd# :&nbsp;<b>".($arr["cds"] == 0 ? "Unknown" : ($arr["cds"] == 255 ? "More than 5 " : safe($arr["cds"])) )."</b></td></tr>
<tr><td align='left'>Hits :&nbsp;<b>{$arr["hits"]}</b></td></tr>
<tr><td align='left'>Uploader :&nbsp;<b><a href='userdetails.php?id=".$arr["owner"]."' target='_blank'>{$arr["username"]}</a></b>&nbsp;&nbsp;";
...
$HTMLOUT .= end_main_frame();
print stdhead("Details for ".safe($arr["name"])."") . $HTMLOUT . stdfoot();
}
}
elseif ($mode== "preview")
{
...
$HTMLOUT .="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"
		...
		<head>
		<title>Preview for - ".$arr["name"]."</title>
		</head>
		<body>
	<div style='font-size:12px;color:black;background-color:#CCCCCC;'>Subtitle preview<br />".safe($fileContent)."</div>
	</body></html>";
print $HTMLOUT;
}
}
...
$HTMLOUT .="<tr><td style='border:none;'>Search result for <i>'{$s}'</i><br />".(mysql_num_rows($res) == 0 ? "Nothing found! Try again with a refined search string." : "" )."</td></tr>";
}
$HTMLOUT .="
</table>
<br />";
...
<td><a href='subtitles.php?mode=details&amp;id=".$arr["id"]."' onmouseover=\"tip('<img src=\'".$arr["poster"]."\' width=\'100\'>')\" onmouseout=\"untip()\">".safe($arr["name"])."</a></td>
<td align='center'><a href='".safe($arr["imdb"])."'  target='_blank'><img src='pic/imdb.gif' border='0' alt='Imdb' title='Imdb' /></a></td>
...
$HTMLOUT .="<td align='center'><a href='userdetails.php?id=".$arr["owner"]."'>".safe($arr["username"])."</a></td></tr>";
}
$HTMLOUT .="</table>";
}

$HTMLOUT .= end_frame();
print stdhead("Subtitles") . $HTMLOUT . stdfoot();
}
?>