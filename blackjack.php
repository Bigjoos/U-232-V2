...
if ($game)
{
	...
	if ($_POST["game"] == 'hit')
	{
      ...
      //autoshout($message);
		...
		elseif ((isset($_POST["start"]) != 'yes' && isset($_POST['continue']) != 'yes') && !$gameover)
		{
		...
		if ($points == 21)
		{
          ...
          $mc1->update_row(false, array('uploaded' => $update['uploaded'], 'bjwins' => $update['bjwins']));
          $mc1->commit_transaction($INSTALLER09['expires']['curuser']);
          $mc1->begin_transaction('MyUser'.$a['userid']);
          $mc1->update_row(false, array('uploaded' => $update['uploaded_loser'], 'bjlosses' => $update['bjlosses']));
          $mc1->commit_transaction($INSTALLER09['expires']['curuser']);
					//==pms
