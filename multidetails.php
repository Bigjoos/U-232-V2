...
if (array_key_exists('uploaded', $_GET) && $_GET["uploaded"]) {
    ...
    $res = sql_query("SELECT torrents.filename FROM torrents WHERE torrents.id=$ids[0] OR torrents.id=$ids[1] OR torrents.id=$ids[2] OR torrents.id=$ids[3] OR torrents.id=$ids[4];") or sqlerr();
    ...
    $res = sql_query("SELECT * FROM torrents WHERE torrents.id=$ids[0] OR torrents.id=$ids[1] OR torrents.id=$ids[2] OR torrents.id=$ids[3] OR torrents.id=$ids[4];") or sqlerr();
    $HTMLOUT .="<tr><td class='colhead'><b>Torrent Description</b></td><td class='colhead'><img src=\"{$INSTALLER09['pic_base_url']}download.gif\" alt=\"Download\" title=\"Download\" border=\"0\" /></td></tr>";
    $i = 0;
    while ($row = mysql_fetch_array($res)) {
...
print stdhead("Multi-Details") . $HTMLOUT . stdfoot();
