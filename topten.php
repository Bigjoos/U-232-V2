...
$HTMLOUT .= "$imgstartpie&amp;chd=t:$tot1,$tot2,$tot3,$tot4,$tot5,$tot6,$tot7,$tot8,$tot9,$tot10&amp;chl=$tor1($tot1)|$tor2($tot2)|$tor3($tot3)|$tor4($tot4)|$tor5($tot5)|$tor6($tot6)|$tor7($tot7)|$tor8($tot8)|$tor9($tot9)|$tor10($tot10)\" alt='' />";	
 ...
 $HTMLOUT .="<h2>Top 10 Countries (users)</h2>";
	  ...
	  if($counted == "10"){
		...
		$name10		=	$arr[9]["name"];
		$num10		=	$arr[9]["num"];	
$HTMLOUT .= "$imgstartbar&amp;chds=0,$num1&amp;chxr=1,0,$num1&amp;chd=t:$num1,$num2,$num3,$num4,$num5,$num6,$num7,$num8,$num9,$num10&amp;chxt=x,y,x&amp;chxl=0:|$name1|$name2|$name3|$name4|$name5|$name6|$name7|$name8|$name9|$name10|2:|($num1)|($num2)|($num3)|($num4)|($num5)|($num6)|($num7)|($num8)|($num9)|($num10)\"  alt='' />";	
	  } else {
...
$HTMLOUT .= "<br /><br /><h2>Top 10 Fastest Uploaders</h2>";
	$result 	=	sql_query("SELECT  username, uploaded / (".time()." - added) AS upspeed FROM users WHERE enabled = 'yes' ORDER BY upspeed DESC LIMIT 10");
	  $counted		=	mysql_num_rows($result);
	  if($counted == "10"){
		$arr = mysql_fetch_rowsarr($result);
		$user1	 	= $arr[0]['username'];
